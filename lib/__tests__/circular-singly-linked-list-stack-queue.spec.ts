import { CircularSinglyLinkedListStackQueue } from '../modules'

describe('TEST CIRCULAR-SINGLY-LINKED-LIST-STACK-QUEUE', () => {
  it('should convert queue into an array', () => {
    const queue = new CircularSinglyLinkedListStackQueue()
  ...
  it('should check whether the queue is empty', () => {
    const emptyQueue = new CircularSinglyLinkedListStackQueue()
    const filledQueue = new CircularSinglyLinkedListStackQueue().add(1)
    expect([emptyQueue.isEmpty(), filledQueue.isEmpty()]).toEqual([true, false])
  })

  it('should clear all elements from queue', () => {
    const queue = new CircularSinglyLinkedListStackQueue().add(1).add(2).clear()
    expect(queue.isEmpty()).toEqual(true)
  })

  it('should reverse the elements inside queue', () => {
    const queue = new CircularSinglyLinkedListStackQueue().add(1).add(2).add(3)
